...
import {
  createTRPCRouter,
  isRateLimited,
  isPromptImprovementRateLimited,
  protectedProcedure,
...
export const imagesRouter = createTRPCRouter({
  ...
  getAll: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const result = await GeneratedImages.getAllImages(ctx.user.id, {
        search,
        limit,
        page: cursor,
      });
      const images = result.images.map((x) => ({
        ...x,
        url: FileHandler.getImageUrl(x.key),
      }));
  ...
  generateImage: protectedProcedure
    ...
    .mutation(async ({ input: { prompt }, ctx }) => {
      try {
        const userAccount = await UserAccounts.getOrCreateUserAccount(
          ctx.user.id
        );

        if (
          !userAccount.isUnlimited &&
          userAccount.imageGenerationTokens <= 0
        ) {
          throw new TRPCError({
            code: "BAD_REQUEST",
        ...
        for (const upload of uploadResults) {
          ...
          try {
            ...
            const buffer = await sharpImage.raw().ensureAlpha().toBuffer();

            const bytes = new Uint8ClampedArray(buffer);
            const resultBlurHash = blurhash.encode(
              bytes,
              imageMetadata.width as number,
              imageMetadata.height as number,
              4,
              4
            );
        ...
        if (!userAccount.isUnlimited) {
          ...
          await UserAccounts.decrementTokenCount(
            ctx.user.id,
  ...
  improvePrompt: protectedProcedure
    ...
    .mutation(async ({ input: { prompt }, ctx }) => {
      try {
        ...
        const improvedPrompt = await AI.improveImagePrompt({
          prompt,
          userId: ctx.user.id,
        });
